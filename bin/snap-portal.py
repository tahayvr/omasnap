...
argv = sys.argv[1:]
mode = argv.pop(0) if argv and argv[0] in ("open", "save") else "open"
start = argv[0] if argv and argv[0] else os.path.expanduser("~/Pictures")
...
jpeg = name.lower().endswith((".jpg", ".jpeg"))
png = ("PNG", [(0, "*.png"), (0, "*.PNG")])
jpg = ("JPEG", [(0, "*.jpg"), (0, "*.jpeg"), (0, "*.JPG")])

options = {
    "handle_token": GLib.Variant("s", token),
    "accept_label": GLib.Variant("s", "Save" if mode == "save" else "Open"),
    "modal": GLib.Variant("b", True),
    "current_folder": GLib.Variant("ay", start.encode() + b"\0"),
}
if mode == "save":
    # The matching filter first, so the chooser does not offer to rename a
    # JPEG to .png behind the suggested name.
    options["current_name"] = GLib.Variant("s", name)
    options["filters"] = GLib.Variant("a(sa(us))", [jpg, png] if jpeg else [png, jpg])
else:
    options["multiple"] = GLib.Variant("b", False)
