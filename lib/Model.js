...
// settings.json carries the version of its layout. A change to what a key
// means, or a key renamed or split, bumps SETTINGS_VERSION and adds the step
// from the version before to SETTINGS_MIGRATIONS; a plain new key needs
// neither, since cleanSettings gives it its default. A file without a version
// is 0: what the settings view first wrote.
var SETTINGS_VERSION = 1;

// SETTINGS_MIGRATIONS[n] takes a file at version n to n + 1, and only has to
// know those two.
var SETTINGS_MIGRATIONS = [
    function (o) { return o; }       // 0 → 1: the version field itself
];

function settingsVersion(o) {
    var v = o && typeof o === "object" ? o.version : undefined;
    return typeof v === "number" && isFinite(v) && v >= 0 ? Math.floor(v) : 0;
}

// Settings as read off disk, brought up to this version. A file from a newer
// Postcard is read as far as it can be, and marked so it is never written
// over: going back a version must not lose what the newer one saved.
function migrateSettings(o) {
    var from = settingsVersion(o);
    var cur = o && typeof o === "object" ? JSON.parse(JSON.stringify(o)) : {};
    for (var v = from; v < SETTINGS_VERSION; v++) cur = SETTINGS_MIGRATIONS[v](cur) || {};
    return { settings: cleanSettings(cur), from: from, tooNew: from > SETTINGS_VERSION };
}

// What is written: the version first, then every setting.
function settingsFile(values) {
    var out = { version: SETTINGS_VERSION };
    var clean = cleanSettings(values);
    for (var k in clean) out[k] = clean[k];
    return out;
}
