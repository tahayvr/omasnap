...
function hitAnnotation(a, px, py, slop) {
    var r = normalised(a);
...
// The corner opposite the one in hand stays put, and a step badge keeps its
// two sides equal so it stays round.
function resizeAnnotation(a, key, px, py) {
    ...
    var r = normalised(a);
    var ax = (key === "tl" || key === "bl") ? r.x + r.w : r.x;
    var ay = (key === "tl" || key === "tr") ? r.y + r.h : r.y;
