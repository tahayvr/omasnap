...
function newAnnotation(kind, x, y) {
    return {
        ...
        kind: kind,
        x: x, y: y, w: 0, h: 0,
        color: "",                 // "" means "use the current ink colour"
        width: 3,
        text: "",
        index: 0,
...
// The title bar borrows the card's own dominant colour so the two read as one
...
// edge colour, so the screenshot reads as having more room inside its window.
